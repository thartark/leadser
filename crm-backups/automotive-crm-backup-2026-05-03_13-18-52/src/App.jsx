...
import { LayoutDashboard, Users, Car, Wrench, BarChart3, Settings, ChevronLeft, Search, Bell, Sun, Moon, Phone, Mail, Calendar, MessageSquare, X, Clock, TrendingUp, TrendingDown, Target, AlertCircle, Info, Check, Filter, Activity, DollarSign, Download } from 'lucide-react';
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${config[status] || "bg-slate-100"}`}>
      {status}
    </span>
  );
};
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => onAction("Call", lead)} className="flex items-center justify-center gap-2 px-3 py-2 bg-slate-100 rounded-lg text-sm">
                <Phone size={14} /> Call
              </button>
              <button onClick={() => onAction("Text", lead)} className="flex items-center justify-center gap-2 px-3 py-2 bg-slate-100 rounded-lg text-sm">
                <MessageSquare size={14} /> Text
              </button>
              <button onClick={() => onAction("Email", lead)} className="flex items-center justify-center gap-2 px-3 py-2 bg-slate-100 rounded-lg text-sm">
                <Mail size={14} /> Email
              </button>
              <button onClick={() => onAction("Appointment", lead)} className="flex items-center justify-center gap-2 px-3 py-2 bg-slate-100 rounded-lg text-sm">
                <Calendar size={14} /> Set Appt
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-5 shadow-sm border">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <RePieChart>
              <Pie data={leadSourcesData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
            ...
            {hotLeads.map(lead => (
              <div key={lead.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg cursor-pointer" onClick={() => onSelectLead(lead)}>
                ...
                <div className="flex gap-2">
                  <button onClick={(e) => { e.stopPropagation(); onToast("Calling " + lead.name + "...", "info"); }} className="p-2 bg-emerald-100 rounded-lg">
                    <Phone size={14} className="text-emerald-600" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); onToast("Texting " + lead.name + "...", "info"); }} className="p-2 bg-indigo-100 rounded-lg">
              ...
              {filteredLeads.map(lead => (
                <tr key={lead.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => onSelectLead(lead)}>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs ${lead.score >= 80 ? 'bg-rose-100 text-rose-700' : lead.score >= 60 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100'}`}>
                      {lead.score}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={(e) => { e.stopPropagation(); onToast("Calling " + lead.name, "info"); }} className="p-1.5 hover:bg-slate-100 rounded">
                      <Phone size={14} />
                    </button>
                  </td>
                </tr>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      {inventoryData.map(vehicle => (
        <div key={vehicle.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition-all">
          <div className="h-32 bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-lg font-bold">
          ...
          <div className="p-4">
            ...
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="flex gap-2 text-xs">
                <span className="px-2 py-1 bg-slate-100 rounded">Views: {vehicle.views}</span>
                <span className="px-2 py-1 bg-emerald-100 rounded">Inquiries: {vehicle.inquiries}</span>
              </div>
            </div>
            <button onClick={() => onToast("Matching leads for " + vehicle.make + " " + vehicle.model, "success")} className="w-full py-2 bg-indigo-50 text-indigo-600 rounded-lg text-sm font-medium">
              Match to Lead
            </button>
          </div>
        </div>
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 text-center border">
          ...
          <p className="text-sm text-slate-500">Total ROs</p>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 text-center border">
          <p className="text-2xl font-bold text-indigo-600">{appointments.filter(a => a.status === "In Progress").length}</p>
          <p className="text-sm text-slate-500">In Progress</p>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 text-center border">
          <p className="text-2xl font-bold text-amber-600">{appointments.filter(a => a.status === "Waiting").length}</p>
        ...
        <div className="divide-y divide-slate-100">
          {appointments.map(apt => (
            <div key={apt.id} className="flex items-center justify-between p-4 hover:bg-slate-50">
              ...
              <select value={apt.status} onChange={(e) => updateStatus(apt.id, e.target.value)} className="px-2 py-1 text-sm border rounded-lg">
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold">Sales Performance Report</h3>
        <button onClick={() => onToast("Report exported", "success")} className="flex items-center gap-2 text-indigo-600 text-sm">
      ...
      <table className="w-full">
        <thead className="bg-slate-50">
          <tr>
            <th className="px-4 py-3 text-left text-xs">Rep</th>
            <th className="px-4 py-3 text-left text-xs">Leads</th>
            <th className="px-4 py-3 text-left text-xs">Units</th>
            <th className="px-4 py-3 text-left text-xs">Gross</th>
            <th className="px-4 py-3 text-left text-xs">Close %</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {salespeople.map(rep => (
            <tr key={rep.id}>
              ...
              <td className="px-4 py-3">{rep.leads}</td>
              <td className="px-4 py-3 font-bold">{rep.units}</td>
              <td className="px-4 py-3 text-emerald-600">${(rep.gross/1000).toFixed(0)}K</td>
              <td className="px-4 py-3">{rep.closeRate}%</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="space-y-3">
        {salespeople.map(user => (
          <div key={user.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
            ...
            <button onClick={() => onToast("Editing " + user.name, "info")} className="px-3 py-1 text-indigo-600 text-sm">
              Edit
            </button>
          </div>
        ))}
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {navigation.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentPage === item.id ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-100'}`}
            >
              <item.icon size={20} />
            ...
            <div className="relative">
              ...
              {showNotifications && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl shadow-lg border z-50">
                  ...
                  <div className="p-3"><p className="text-sm">New lead from AutoTrader</p><p className="text-xs text-slate-500">2 min ago</p></div>
                </div>
              )}
            </div>
      ...
      {selectedLead && (
        <LeadDrawer
          ...
          onAction={(action, lead) => addToast(action + " action for " + lead.name, "success")}
        />
      )}
